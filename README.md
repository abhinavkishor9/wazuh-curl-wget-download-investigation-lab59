...
1. Establish the Windows endpoint identity and investigation time.
2. Create a dedicated investigation directory.
3. Establish a filesystem baseline.
4. Prepare a controlled benign download source.
5. Verify the Windows `curl.exe` binary.
6. Execute a controlled curl download.
7. Verify the resulting file.
8. Review Sysmon telemetry.
9. Verify Wazuh agent connectivity.
10. Document telemetry gaps and troubleshooting observations.
11. Build a defensible investigation timeline.
...
A basic detection could identify:
...
However, alerting on curl execution alone would likely generate false positives.

A stronger detection strategy would correlate:
...
- `curl.exe` downloading an executable.
- `curl.exe` downloading a PowerShell script.
- Downloads into `%TEMP%`.
- Downloads followed immediately by execution.
- Downloads from newly observed domains.
- Downloads initiated by unusual parent processes.
- Downloads performed by service accounts.
- Downloads involving suspicious IP addresses.

---

# 15. Lessons Learned

1. `curl.exe` is a legitimate utility and should not automatically be classified as malicious.
2. Process creation telemetry alone provides limited context.
3. A successful HTTP transfer does not guarantee that the intended file was retrieved.
4. File content verification is important after a download.
5. Sysmon Event ID 1 successfully confirmed curl execution.
6. Network connection telemetry would improve the investigation.
7. File creation telemetry would improve correlation.
8. Wazuh agent health should be checked before investigating missing telemetry.
9. "Not observed" should not automatically be interpreted as "not generated."
10. A complete SOC investigation should correlate process, network, file, and execution telemetry.

---

# 16. Final Conclusion

The lab successfully simulated Windows `curl.exe` download activity and confirmed the process through Sysmon Event ID 1. The resulting 334-byte file was created successfully but contained an HTTP 400 error response rather than the intended benign content. The Wazuh agent for the Windows endpoint was confirmed to be active. However, only Sysmon Event ID 1 was successfully obtained, leaving network, file-creation, Windows Security, and PowerShell telemetry gaps. The investigation therefore demonstrates both command-line download analysis and the importance of distinguishing confirmed evidence from telemetry that was not available.

---

## Lab Status

**Completed with telemetry limitations**

Primary investigation chain:
...
Download Attempt
        |
        v
334-byte File Created
        |
        v
HTTP 400 Error Content
        |
        v
Sysmon Event ID 1
        |
...
For a second iteration of this lab, enable and validate:

- Sysmon Event ID 3.
- Sysmon Event ID 11.
- Windows Security Event ID 4688.
- PowerShell Event ID 4104.
- Wazuh collection of the relevant Windows event channels.
- Full command-line telemetry.
- Process parent-child relationships.
- Network destination telemetry.
- File hash collection.

This would allow the investigation to progress from a basic process investigation into a complete:

```text
Process → Network → File → Execution
```
