...
A Windows 11 endpoint is investigated after curl.exe is used to download a file during a controlled SOC lab. Since curl.exe is a legitimate utility but can also be abused for payload delivery, the activity must be validated using available endpoint evidence.

The download creates a 334-byte file, but inspection shows an HTTP 400 error response instead of the expected benign content. Sysmon Event ID 1 confirms curl.exe execution, while the Wazuh agent is confirmed active.

The investigation aims to determine:

- Whether curl.exe executed successfully on the endpoint.
- What file was created and what content it contained.
- What Sysmon and Wazuh telemetry was available.
- Which expected telemetry was missing.
- Whether the available evidence is sufficient to classify the activity as malicious.
